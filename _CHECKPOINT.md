...
- Rollback de produção: promover deployment anterior no painel Vercel ou git `8a11f10` (pré-fix dessincronia) / `47c2a54` (pré-fix B_KEEP) / `e03db4a` (pré-upscale) / `ca3bb70` (pré-filme).
...
4. Dívida técnica pré-existente no FilmScroll (achada pelo Codex 20/07; dessincronia texto/frame já CORRIGIDA — ver Decisões): (a) `createImageBitmap` sem teto de concorrência; (b) rAF do scrub roda permanentemente mesmo com o filme fora do viewport (bateria mobile); (c) `verify-film.mjs` não falha o processo em números ruins, só loga. Nenhuma bloqueia produção hoje; considerar antes do master 1080p ou se houver relato de problema em mobile real.
