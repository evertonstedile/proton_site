...
## ▶ RETOMAR AQUI
Filme 5/5 EM PRODUÇÃO em www.protonsc.com.br (`main` = `redesign-framework` = `88ab691`; deploy verificado no domínio real: 5 shots desktop + 3 mobile + reduced-motion + jank p95 9.1ms/max 17.2ms; suite Playwright 44/44; `verify-film.mjs` agora É gate real, exit 0). Frames upscalados via Real-ESRGAN x4plus local (1280w→1728w, zero créditos) + toda a dívida técnica achada pelo Codex 20/07 corrigida no mesmo dia (memória de bitmap, dessincronia texto/frame, concorrência de bitmap, rAF pausa fora do viewport, harness virou gate). Ver "Decisões". Não há tarefa em andamento nem dívida técnica pendente no FilmScroll. Próximos passos são DECISÕES do Everton, nesta ordem: (1) revisar copy dos beats 4–5 (o que está no ar + alternativas estão em "Decisões" abaixo); (2) master 1080p Seedance real — ~225 cr, precisa recarga + OK explícito (ganho adicional sobre o upscale ESRGAN: motion real gerado em alta res vs. upscale de 480p); ao rodar: re-gerar os 5 clipes com os MESMOS prompts (histórico Higgsfield), re-assemble, trocar `FRAME_COUNT`/`SEAM_HEX` no FilmScroll e re-rodar `node scripts/verify-film.mjs`; (3) Deployment Protection dos previews (produção é pública; previews pedem login — desligar é setting da conta dele).
...
- Achado do Codex também confirmado por bug visual real no mobile: texto do beat (posição de scroll bruta `p`) dessincronizava do frame do canvas (dependia do fetch já ter chegado) durante carregamento em rede lenta — copy de "Estrutura" aparecia sobre frame de "Noite". Pré-existente, não causado pelo upscale, mas o payload maior (41MB) alongava a janela em que era visível. **Corrigido same-day**: progresso EFETIVO `pEff = min(p, loaded/FRAME_COUNT)` vira fonte única pra target de frame, capítulo e beatAlpha — nunca avança além do que carregou; barra de progresso continua no `p` bruto (feedback de scroll, não de conteúdo); beat 0/h1 não regride no LCP (pEff clampado em 0 no primeiro tick já dá alpha=1 pro hero). Jank melhorou (max 17.5ms→16.8ms em produção). Commit `cbbc623`, deployado e verificado.
- Toda a dívida técnica restante do Codex também corrigida same-day (commit `88ab691`): `BMP_CONCURRENCY=6` em `ensureBitmaps` (loop de `createImageBitmap` não estoura mais decodes simultâneos, excesso fica pro próximo tick); `IntersectionObserver` (rootMargin 50%) pausa o loop de scrub (rAF) quando o filme sai do viewport — confirmado com teste de wheel real via Playwright (`current`/`displayed` congelam idênticos após 1s fora da tela, retomam ao reentrar); `verify-film.mjs` agora falha (`process.exit(1)`) se jank max ≥50ms ou reduced-motion fora do esperado — deixou de ser só log. Jank melhorou a cada fix (max 373MB-era→17.5ms→9.4ms dev / 17.2ms prod).
- Rollback de produção: promover deployment anterior no painel Vercel ou git `cbbc623` (pré-fix concorrência/rAF/gate) / `8a11f10` (pré-fix dessincronia) / `47c2a54` (pré-fix B_KEEP) / `e03db4a` (pré-upscale) / `ca3bb70` (pré-filme).
...
- `window.scrollTo()` direto no console NÃO funciona neste site (Lenis intercepta smooth-scroll) — usar sempre o contrato `?jump=<scrollY>` do próprio engine pra testar posições de scroll. `?jump` com valor MUITO grande (fora do range do driver, ex. >15000px) também não gruda — Lenis some com o scroll. Pra testar posições fora do filme, usar Playwright com `page.mouse.wheel()` (wheel real), não scrollTo nem o `computer` scroll do browser pane (trava/timeout neste projeto).
