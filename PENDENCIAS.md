...
| ~~B2~~ ✅ | **Copy real** | RESOLVIDO no redesign framework (13/07/2026): home em 14 movimentos com copy real (HOME_FW), manifesto institucional novo; depoimentos SAÍRAM do fluxo (não publicados). Segue abaixo o histórico: | **REAIS:** serviços (33, c/ escopo), números (371 projetos · 119 laudos · 70.400 m² · 11 anos), posicionamento hero/About (D16), processo (fluxo full-service: Viabilidade → Projeto → Aprovação & Regularização → Execução & Entrega), tagline oficial. Ainda placeholder: **manifesto, depoimentos**. | Home, internas |
| ~~Posicionamento~~ ✅ | **Escopo ampliado** | Hero + About + OG + meta atualizados para o posicionamento oficial full-service ("Soluções completas em engenharia, arquitetura e regularização"). Removida menção ao Proton OS na copy pública. | Hero, About, OG, `lib/site.ts` |
| ~~B3~~ ✅ | **Fotos/vídeo das obras** | RESOLVIDO (13/07/2026): 9 vídeos + 10 fotos entregues e integrados (public/media; masters gitignored). ⚠️ Nomes do zip trocados: fora2.png = aérea do condomínio; image2/image2_noite = par dia/noite do hero. | Home + /obras |
...
| ~~Endereço~~ ✅ | **Logradouro** | CONFIRMADO pelo cliente: Rua Santa Rita, 571 — Centro, Garopaba/SC · 88495-000. | `lib/site.ts` |
...
| ~~B6~~ ✅ | **Domínio definitivo** | RESOLVIDO (DF6, 13/07/2026): `https://protonsc.com.br` é o default em `lib/site.ts`; conferir env NEXT_PUBLIC_SITE_URL na Vercel no deploy. | `lib/site.ts` |
...
| D16 | **Posicionamento ampliado** (aprovado pelo cliente): de "engenharia residencial de alto padrão" para o escopo full-service oficial do catálogo. Hero/About/OG/meta atualizados; menção pública ao Proton OS removida. | Hero, About, OG, meta. |
...
## Redesign framework literal (13/07/2026)

| # | Pendência | Descrição | Onde trava |
|---|-----------|-----------|------------|
| DF7 | **Sócios: fotos + bios** | Aguardando retorno da Proton. Seções /sobre e home publicam SÓ nome + credencial CREA (nada inventado). | Movimento 12, /sobre |
| DF9b | **Wine House: escopo/disciplinas** | Ficha técnica publicada só com o confirmado (local, 91,79 m², 2026, concluída). Escopo/disciplinas: TODO: VALIDAR. | Ficha (home + /obras/winehouse) |
| HG | **Higgsfield (gate)** | Nenhum take gerado. Crossfade WebGL aprovado como piso. Qualquer take pergunta antes (crédito) e loga em content/production/higgsfield-log.md. | Hero |
