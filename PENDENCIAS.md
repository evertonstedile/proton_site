...
| B5 | **Canal de contato (leads)** | RESOLVIDO (mecanismo): **WhatsApp** (wa.me, botões no form/contato/footer) + **e-mail via Web3Forms** (form → e-mail, sem backend, honeypot anti-spam). Falta só você preencher 3 envs: `NEXT_PUBLIC_WHATSAPP` (nº real), `NEXT_PUBLIC_CONTACT_EMAIL`, `NEXT_PUBLIC_WEB3FORMS_KEY` (grátis em web3forms.com). Sem a chave, o form fica em modo demo + WhatsApp. | `ContactForm.tsx`, `lib/site.ts` |
